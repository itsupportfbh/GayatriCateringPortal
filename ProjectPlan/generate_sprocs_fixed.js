...
const tableBlocks = text.split(/CREATE TABLE \[dbo\]\.\[/).slice(1);
console.log('tableBlocks', tableBlocks.length);
const tables = [];
for (const block of tableBlocks) {
  const firstLine = block.split(/\r?\n/)[0];
  const nameMatch = firstLine.match(/^([^\]]+)\]/);
  if (!nameMatch) {
    console.log('no nameMatch for block start:', JSON.stringify(firstLine));
  }
  if (!nameMatch) continue;
  const name = nameMatch[1];
  const rest = block.slice(nameMatch[0].length);
  const endIndex = rest.indexOf(') ON [PRIMARY]');
  if (endIndex === -1) continue;
  const colsText = rest.slice(0, endIndex);
  const cols = [];
  for (const rawLine of colsText.split(/\r?\n/)) {
    ...
    line = line.replace(/,$/, '');
    const colm = line.match(/^\[([^\]]+)\]\s+\[?([A-Za-z0-9]+)\]?(?:\(([^\)]*)\))?(.*)$/);
    if (!colm) continue;
    const colname = colm[1];
    const baseType = colm[2];
    const size = colm[3];
    const ctype = size ? `${baseType}(${size})` : baseType;
    const restCols = colm[4];
    ...
    const isIdentity = restCols.includes('IDENTITY');
    const isNullable = !restCols.includes('NOT NULL');
    cols.push({ colname, ctype, isNullable, isIdentity });
  }
  const pkMatch = block.match(/PRIMARY KEY CLUSTERED \([^\)]*\[([^\]]+)\] ASC/);
  let pk = pkMatch ? pkMatch[1] : (cols.find(c => c.isIdentity) || cols[0]);
  if (pk && typeof pk !== 'string') pk = pk.colname;
  if (!pk) continue;
  tables.push({ name, pk, cols });
}
