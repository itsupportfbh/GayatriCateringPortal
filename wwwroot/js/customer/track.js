$(function () {
    ...
    $('#btnTrack').on('click', function () {
        ...
        $.get('/Customer/TrackOrder?id=' + encodeURIComponent(reference), function (data) {
            renderTrack(data);
        }).fail(function () {
    ...
    function renderTrack(order) {
        var stepIndex = statusSteps.indexOf(order.status);
        var stepsHtml = statusSteps.map(function (step, index) {
            var cssClass = index < stepIndex ? 'done' : (index === stepIndex ? 'active' : '');
            return '<div class="wizard-step ' + cssClass + '" style="flex:1;padding:10px;text-align:center;font-size:12px;font-weight:700;border:1px solid var(--gray-200);background:' +
                (index === stepIndex ? 'var(--green)' : index < stepIndex ? 'var(--green-light)' : 'var(--white)') +
                ';color:' + (index === stepIndex ? '#fff' : index < stepIndex ? 'var(--green)' : 'var(--gray-500)') + '">' + step + '</div>';
        }).join('');
